...
echo "Pushing {{ cookiecutter.__project_slug }}:latest to Roboto's private registry"
image_push_args=(
...
if [ $image_push_ret_code -ne 0 ]; then
    echo "Failed to push {{ cookiecutter.__project_slug }}:latest to Roboto's private registry"
    exit 1
fi
